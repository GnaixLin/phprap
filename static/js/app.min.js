    ...
    $.fn.validateForm = function(config) {
        var defaultconfig = {
            ...
            success: function(data) {
                parent.location.reload(data.callback)
            },
            error: ""
        };
        var thisobj = $(this);
        var elebtn = $.extend(defaultconfig, config);
        var submitbtn = elebtn.submitBtn;
        var actbefore = elebtn.before;
        var actsuccess = elebtn.success;
        var acterror = elebtn.error;
        thisobj.Validform({
            tiptype: function(msg, element) {
                if (!element.obj.is("form")) {
                    if (3 == element.type) {
                        alert(msg, "error");
                    }
                }
            ...
            callback: function(data) {
                if (data.status == "success") {
                    if (data.message) {
                        alert(data.message, "success", function() {
                            actsuccess && actsuccess(data)
                        })
                    } else {
                        actsuccess && actsuccess(data)
                    }
                } else {
                    acterror && acterror(data);
                    $(".js_" + data.label).focus().addClass("Validform_error");
                    if (data.message) {
